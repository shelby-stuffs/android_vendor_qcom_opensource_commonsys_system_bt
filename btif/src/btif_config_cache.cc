...
const std::unordered_set<std::string> kLinkKeyTypes = {
    ...
    "LE_KEY_PCSRK", "LE_KEY_LENC", "LE_KEY_LCSRK", "LE_KEY_LID" };
...
void BtifConfigCache::SetUint16(std::string section_name, std::string key,
                                uint16_t value) {
  SetString(std::move(section_name), std::move(key), std::to_string(value));
}

std::optional<uint16_t> BtifConfigCache::GetUint16(
    const std::string& section_name, const std::string& key) {
  auto value = GetString(section_name, key);
  if (!value) {
    return std::nullopt;
  ...
  char* endptr;
  uint16_t ret = strtoull(value->c_str(), &endptr, 0);
  if (*endptr != '\0') {
    LOG(WARNING) << "Failed to parse value to uint64 for section "
                 << section_name << ", key " << key;
    return std::nullopt;
