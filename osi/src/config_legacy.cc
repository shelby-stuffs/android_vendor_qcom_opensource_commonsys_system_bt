...
#define LOG_TAG "bt_osi_config_legacy"
...
struct config_legacy_section_iter_t {};

static bool config_legacy_parse(FILE* fp, config_legacy_t* config);

static section_legacy_t* section_new(const char* name);
static void section_free(void* ptr);
static section_legacy_t* section_find(const config_legacy_t* config, const char* section);

static entry_legacy_t* entry_new(const char* key, const char* value);
static void entry_free(void* ptr);
static entry_legacy_t* entry_find(const config_legacy_t* config, const char* section,
                           const char* key);

config_legacy_t* config_legacy_new_empty(void) {
  config_legacy_t* config = static_cast<config_legacy_t*>(osi_calloc(sizeof(config_legacy_t)));
...
config_legacy_t* config_legacy_new(const char* filename) {
  ...
  config_legacy_t* config = config_legacy_new_empty();
  ...
  if (!config_legacy_parse(fp, config)) {
    config_legacy_free(config);
    config = NULL;
  }
...
config_legacy_t* config_legacy_new_clone(const config_legacy_t* src) {
  ...
  config_legacy_t* ret = config_legacy_new_empty();

  CHECK(ret != NULL);

  for (const list_node_t* node = list_begin(src->sections);
       node != list_end(src->sections); node = list_next(node)) {
    section_legacy_t* sec = static_cast<section_legacy_t*>(list_node(node));

    if (sec) {
      for (const list_node_t* node_entry = list_begin(sec->entries);
           node_entry != list_end(sec->entries);
           node_entry = list_next(node_entry)) {
        entry_legacy_t* entry = static_cast<entry_legacy_t*>(list_node(node_entry));

        config_legacy_set_string(ret, sec->name, entry->key, entry->value);
      }
    }
  }
...
int config_legacy_get_int(const config_legacy_t* config, const char* section, const char* key,
  ...
  entry_legacy_t* entry = entry_find(config, section, key);
...
unsigned short int config_legacy_get_uint16(const config_legacy_t* config, const char* section, const char* key,
                   uint16_t def_value) {
  CHECK(config != NULL);
  CHECK(section != NULL);
  CHECK(key != NULL);
...
void config_legacy_set_bool(config_legacy_t* config, const char* section, const char* key,
  ...
  config_legacy_set_string(config, section, key, value ? "true" : "false");
}

void config_legacy_set_string(config_legacy_t* config, const char* section, const char* key,
                       const char* value) {
  section_legacy_t* sec = section_find(config, section);
  if (!sec) {
  ...
  if (sec) {
    for (const list_node_t* node = list_begin(sec->entries);
      ...
      entry_legacy_t* entry = static_cast<entry_legacy_t*>(list_node(node));
    ...
    entry_legacy_t* entry = entry_new(key, value_no_newline.c_str());
...
const char* config_legacy_section_name(const config_section_node_t* node) {
  CHECK(node != NULL);
  const list_node_t* lnode = (const list_node_t*)node;
  const section_legacy_t* section = (const section_legacy_t*)list_node(lnode);
  return section->name;
}
...
section_legacy_t* config_legacy_section(const config_section_node_t* node) {
  CHECK(node != NULL);
  const list_node_t* lnode = (const list_node_t*)node;
  section_legacy_t* section = (section_legacy_t*)list_node(lnode);
  return section;
}
...
bool section_has_key(const section_legacy_t* section,
                    const char* key) {
  CHECK(section != NULL);
  CHECK(key != NULL);
...
void config_legacy_sections_sort_by_entry_key(config_legacy_t* config, compare_func comp) {
  ...
  for (list_node_t* node = list_begin(config->sections);
      ...
      node = list_next(node)) {
    section_legacy_t* sec = (section_legacy_t*)list_node(node);
    ...
    while (p != head_next && changed) {
      ...
      for (;list_next(q) && list_next(q) != p; q = list_next(q)) {
        entry_legacy_t* first = (entry_legacy_t*)list_node(q);
        entry_legacy_t* second = (entry_legacy_t*)list_node(list_next(q));
        char* tmp_key;
        char* tmp_value;
...
bool config_legacy_save(const config_legacy_t* config, const char* filename) {
  ...
  for (const list_node_t* node = list_begin(config->sections);
    ...
    const section_legacy_t* section = (const section_legacy_t*)list_node(node);
    if (section->name[0] == '#') {
    ...
    for (const list_node_t* enode = list_begin(section->entries);
      ...
      const entry_legacy_t* entry = (const entry_legacy_t*)list_node(enode);
  ...
  while (fgets(line, sizeof(line), fp)) {
    ...
    if (*line_ptr == '#') {
        ...
        if(!section_find(config, comment)) {
            section_legacy_t *sec = section_new(comment);
            if (sec)
                list_append(config->sections, sec);
        }
    } else if (*line_ptr == '[') {
      ...
      *split = '\0';
      config_legacy_set_string(config, section, trim(line_ptr), trim(split + 1));
    }
  }
  return true;
}
...
static section_legacy_t* section_new(const char* name) {
  section_legacy_t* section = static_cast<section_legacy_t*>(osi_calloc(sizeof(section_legacy_t)));
...
static void section_free(void* ptr) {
  ...
  section_legacy_t* section = static_cast<section_legacy_t*>(ptr);
...
static entry_legacy_t* entry_new(const char* key, const char* value) {
  entry_legacy_t* entry = static_cast<entry_legacy_t*>(osi_calloc(sizeof(entry_legacy_t)));
...
static void entry_free(void* ptr) {
  ...
  entry_legacy_t* entry = static_cast<entry_legacy_t*>(ptr);
  osi_free(entry->key);
  osi_free(entry->value);
  osi_free(entry);
}
